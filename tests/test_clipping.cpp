...
TEST_CASE("Clip polygon at a plane", "[clipping]") {
    ...
    auto normal = normalize(Normal3f{-1, 1, 0});
...
TEST_CASE("Simple line clipping test", "[clipping]") {
    ...
    REQUIRE(p0 == (Vector3f{0, 0, -1}));
    REQUIRE(p1 == (Vector3f{0, 0, 1}));

    p0 = {0, -10, 0};
    p1 = {0, 10, 0};

    REQUIRE(clip_line_aabb(p0, p1, box));
    REQUIRE(p0 == (Vector3f{0, -1, 0}));
    REQUIRE(p1 == (Vector3f{0, 1, 0}));

    p0 = {-10, 0, 0};
    p1 = {10, 0, 0};

    REQUIRE(clip_line_aabb(p0, p1, box));
    REQUIRE(p0 == (Vector3f{-1, 0, 0}));
    REQUIRE(p1 == (Vector3f{1, 0, 0}));

    p0 = {-10, 10, 0};
    p1 = {10, 10, 0};

    REQUIRE(!clip_line_aabb(p0, p1, box));
    REQUIRE(p0 == (Vector3f{-10, 10, 0}));
    REQUIRE(p1 == (Vector3f{10, 10, 0}));
}
