...
TEST_CASE("Form factor of two parallel unit squares one unit apart",
    ...
    // cf. [CW93], Figure 4.22.
    auto bottom_left =
        test_triangle({0.5, -0.5, 0}, {0.5, 0.5, 0}, {-0.5, 0.5, 0}); // 0
    auto bottom_right =
        test_triangle({0.5, -0.5, 0}, {-0.5, 0.5, 0}, {-0.5, -0.5, 0}); // 1
    auto top_left =
        test_triangle({-0.5f, 0.5f, 1}, {0.5, 0.5, 1}, {0.5, -0.5, 1}); // 2
    auto top_right =
        test_triangle({-0.5f, 0.5f, 1}, {0.5, -0.5, 1}, {-0.5, -0.5, 1}); // 3
    ...
    float F_01_2 = bottom_right.area() * F_0_2 + bottom_left.area() * F_1_2;
    float F_01_3 = bottom_right.area() * F_0_3 + bottom_left.area() * F_1_3;
    float F_01_23 = F_01_2 + F_01_3;

    constexpr float F_EXPECTED = 0.1998; // cf. [CW93], 4.12.
    REQUIRE(is_within_tolerance(F_01_23, F_EXPECTED, TOLERANCE));
    ...
    float F_23_0 = top_right.area() * F_2_0 + top_left.area() * F_3_0;
    float F_23_1 = top_right.area() * F_2_1 + top_left.area() * F_3_1;
    float F_23_01 = F_23_0 + F_23_1;

    REQUIRE(is_within_tolerance(F_23_01, F_EXPECTED, TOLERANCE));
}
