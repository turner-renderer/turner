...
TEST_CASE("Segment plane intersection", "[intersection]") {
    float t;
    REQUIRE(intersect_segment_plane({0, 0, 0}, {2, 0, 0}, {1, 0, 0}, 1, t));
    REQUIRE(t == 0.5f);
}
...
TEST_CASE("Test intersect plane with AABB", "[intersection]") {
    REQUIRE(intersect_plane_box({1, 0, 0}, 1, {{-10, -10, -10}, {10, 10, 10}}));
    REQUIRE(
        !intersect_plane_box({1, 0, 0}, 20, {{-10, -10, -10}, {10, 10, 10}}));
    REQUIRE(intersect_plane_box({1, 0, -1}, 0, {{9, 9, 9}, {10, 10, 10}}));
}
