...
public:
    ...
    void update(const size_t current) {
        ...
        out_ << "\r" << std::setw(20) << std::setfill(' ') << std::left
             << label_;
        for (int step = 0; step < bar_width; ++step) {
            out_ << "■";
        }
        for (int step = 0; step < 20 - bar_width; ++step) {
            out_ << "□";
        }
        out_ << std::setw(7) << std::setfill(' ') << std::right << std::fixed
